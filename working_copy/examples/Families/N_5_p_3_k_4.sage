...
weights = [k]
a_ells_dict = {k: [a_ells_f]}
unstabilize = {4: True}
ZmodpM = Zmod(p ** (M+1))
weights_in_w = dict(zip(weights, [(ZmodpM(1+p) ** (wt-2) - 1).lift() / p for wt in weights]))

print "This script computes the Hida family of the unique newform of level {3} and weight {4} at p = {0} with {1} moments and w-adic precision {2}.".format(p, M, var_prec, N, k)
...
DD = FamiliesOfOverconvergentDistributions(0, base_coeffs=Qp(p, M), prec_cap=[M,var_prec])
MM = FamiliesOfOMS(N * p, r, coefficients=DD)
...
print "Killing Eisenstein componant."
sys.stdout.flush()
before = walltime()
